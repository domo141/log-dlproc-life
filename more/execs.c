 ...
 test "${1-}" || set -- -O2
 #test "${1-}" || set -- -ggdb
 set -x; exec ${CC:-gcc} -std=c99 -DWERROR "$@" -o "$trg" "$0"
 exit $?
 */
#endif
...
#define _GNU_SOURCE 1 // for execvpe()
#endif
...
//extern char ** environ; _GNU_SOURCE defines

//int main(int argc, char * argv[])
int main(void)
{
    if (chdir("/bin") < 0) err(1, "chdir");

    const char * args[] = { "echo", "Hello", "World", NULL };

    if (! fork_n_wait()) {
        execve("./echo", discard_const_p(char *, args), environ);
        err(1, "not reached");
    }
    if (! fork_n_wait()) {
        execv("./echo", discard_const_p(char *, args));
        err(1, "not reached");
    }
    if (! fork_n_wait()) {
        execvp("./echo", discard_const_p(char *, args));
        err(1, "not reached");
    }
    if (! fork_n_wait()) {
        execvpe("./echo", discard_const_p(char *, args), environ);
        err(1, "not reached");
    }

    if (! fork_n_wait()) {
        execl("./echo", "echo", "Hello", "World", NULL);
        err(1, "not reached");
    }
    if (! fork_n_wait()) {
    ...
    if (! fork_n_wait()) {
        execle("./echo", "echo", "Hello", "World", NULL, environ);
        err(1, "not reached");
    }
    chdir("/");
